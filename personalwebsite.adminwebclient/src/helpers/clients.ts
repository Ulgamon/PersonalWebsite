...
export class Client {
  ...
  protected processBlogPostsGET2(
    response: Response
  ): Promise<ReturnBlogPostDto> {
    const status = response.status;
    let _headers: any = {};
    if (response.headers && response.headers.forEach) {
      response.headers.forEach((v: any, k: any) => (_headers[k] = v));
  ...
  protected processCategoriesAll(
    response: Response
  ): Promise<ReturnCategoriesDto[]> {
    const status = response.status;
    let _headers: any = {};
